...
handlers._listPhone.post = function(data, callback) {
    ...
    if (title && phones) {
        ...
        phones.forEach(function (phoneItem) {
            var phone = typeof (phoneItem) == 'string' && phoneItem.length == 10 ? phoneItem : false;
            if (phone) {
                phonesSuccess.push(phone);
            }  else {
                phonesFailed.push(phoneItem);
            }
        });
