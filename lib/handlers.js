...
var maxsolutions = require('../app/maxsolutions');
...
handlers._users.post = function(data, callback) {
    ...
    var username = typeof(data.payload.username) == 'string' && data.payload.username.trim().length == 10 ? data.payload.username.trim() : false;
    ...
    if (firstName && lastName && username && phone && email && password) {
        ...
        if(hashedPassword){
            ...
            _data.find('users', {username : userObject.username}, function (err) {
                 if (err) {
                     ...
                     callback(400,{'Error' : 'username has exist'});
                 }
            });
        } else {
...
handlers._tokens.post = function(data,callback){
    ...
    if(username && password){
        ...
        _data.find("users", {username : username}, function (err, result) {
...
handlers.smsTracking = function(data, callback) {
    var acceptableParams = ['smsLogs','listPhone','templateSms'];
    ...
    if (path == 'listPhone') {
        handlers.listPhone(data, callback);
    }
    if (path == 'templateSms') {
...
handlers._smsLogs.delete = function(data, callback) {
    ...
    if (id) {
        // Lookup the token
        _data.read('sms/smsLogs',id,function(err,smsLogs){
            if(!err && smsLogs){
                // Delete the token
...
// Sms ListCustomers
handlers.listPhone = function(data, callback) {
    var acceptableMethods = ['post','get','put','delete'];
    if(acceptableMethods.indexOf(data.method) > -1) {
        handlers._listPhone[data.method](data,callback);
    } else {
        callback(405, {"Error :" : "ListCustomers Not support for this method"});
    }
};

// Container for all the listcustomers
handlers._listPhone = {};


// Sms List phone - post
// Required data : phoneList (array)
handlers._listPhone.post = function(data, callback) {
    // Check the required field
    var currentUser = data.currentUser ? data.currentUser : false;

    // Check that all required fields are filled out
    var title = typeof (data.payload.title) == 'string' && data.payload.title.trim().length > 0 ? data.payload.title.trim() : false;
    var phones = typeof (data.payload.phones) == 'object' && data.payload.phones instanceof Array && data.payload.phones.length > 0 ? data.payload.phones : false;

    // Optional data
    var description = typeof (data.payload.description) == 'string' && data.payload.description.trim().length >= 0 ? data.payload.description.trim() : null;

    if  (currentUser) {
        if (title && phones) {
            // var acceptFirstPhoneNumber = [
            //     '83','84','85','81','82', //// Vinaphone
            //     '32','33','34','35','36','37','38','39', // Viettel
            //     '70','79','77','76','78', // Mobiphone
            //     '58','56', // Vietnamobile
            //     '59' // Gtel
            // ];
            ...
            var phonesSuccess = [];
            var phonesFailed = [];
            // Check all that phone is valid
            phones.forEach(function (item) {
                var check = helpers.checkPhoneNumber(item);
                if (check) {
                    phonesSuccess.push(item);
                } else {
                    phonesFailed.push(item);
                }
            });
            ...
            // Check the optional data
            if (description) {
                phonesObject.description = description;
            }

            if (phonesFailed.length === 0) {
                _data.create('sms/listPhone', id, phonesObject, function (err) {
                    if (!err) {
                        callback(200, phonesObject);
                    } else {
                        callback(400, {"Error : " : "Cannot create the phone list"});
                    }
                });
            } else {
                var result = {
                    success : phonesSuccess,
                    failed : phonesFailed
                };
                callback(400, result);
            }
        ...
        callback(400, {"Error:" : "Cannot get the current user"});
    }
};
...
// Sms List phone - get
// Required data : id
handlers._listPhone.get = function(data, callback) {
    var id = typeof (data.queryStringObject.id) == "string" && data.queryStringObject.id.trim().length == 10 ? data.queryStringObject.id.trim() : false;

    if (id) {
        _data.read("sms/listPhone", id, function (err, phones) {
            if (!err && phones) {
                callback(200, phones);
            } else {
                callback(400, {"Error : " : "Cannot get the sms log"});
...
// Sms List phone - put
// Required data : id
// Optional data : phones, title, description
handlers._listPhone.put = function(data, callback) {
    // Check the required field
    var id = typeof (data.queryStringObject.id) == "string" && data.queryStringObject.id.trim().length == 10 ? data.queryStringObject.id.trim() : false;
    ...
    var phones = typeof (data.payload.phones) == "object" && data.payload.phones instanceof Array ? data.payload.phones : false;
    if  (id) {
        if (title || description || phones) {
            // Lookup the phone list
            _data.read('sms/listPhone',id,function(err, listPhone){
                if(!err && listPhone){
                    // Check the optional data
                    if (title) {
                        listPhone.title = title;
                    }
                    if (description) {
                        listPhone.description = description;
                    }
                    if (phones) {
                        listPhone.phones = phones;
                    }
                    listPhone.updated_at = new Date(Date.now());

                    _data.update('sms/listPhone', id, listPhone, function (err) {
                        if (!err) {
                            callback(200, listPhone);
                        } else {
                            callback(400,{'Error' : 'Could not update the specified listPhone.'});
                        }
                    });
                } else {
                    callback(400,{'Error' : 'Could not find the specified listPhone.'});
                }
            });
        }
    }else {
...
// Sms List phone - delete
// Required data : id
handlers._listPhone.delete = function(data, callback) {
    var id = typeof (data.queryStringObject.id) == "string" && data.queryStringObject.id.trim().length == 10 ? data.queryStringObject.id.trim() : false;
    if (id) {
        // Lookup the list phone
        _data.read('sms/listPhone',id,function(err,listPhone){
            if(!err && listPhone){
                // Delete the listPhone
                _data.delete('sms/listPhone',id,function(err){
                    if(!err){
                        callback(200);
                    } else {
                        callback(500,{'Error' : 'Could not delete the specified listPhone'});
                    }
                });
            } else {
                callback(400,{'Error' : 'Could not find the specified listPhone.'});
...
handlers.checkConnection = function(data, callback) {
    maxsolutions.getStatus(function (err, xml) {
        if (!err && xml) {
            // var json = helpers.xmlTagsToJson(['Code','Message','TotalSuccessSms','TotalFailSms'], xml);
            callback(200, xml, 'xml', {token : data.headers.token});
        } else {
            callback(400,err);
        }
    });
