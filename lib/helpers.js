...
helpers.checkPhoneNumber = function(phoneItem) {
    var acceptNumber = [10,11];
    var check = typeof (phoneItem) == "string" &&
        acceptNumber.includes(phoneItem.trim().length) ? true : false;
...
helpers.xmlTagsToJson = function(keys, xml) {

    var keys = typeof (keys) == "object" && keys instanceof Array && keys.length > 0 ? keys : false;
